...
class Display():
	...
	def __init__(self,rows,columns,sizeOfCase):	
		...
		for k in range(0,self._rows):
			self._damier.create_line(self._sizeOfCase*k, 0,self._sizeOfCase*k,self._rows*self._sizeOfCase, width=1)
			self._damier.create_line(0, self._sizeOfCase*k,self._columns*self._sizeOfCase ,self._sizeOfCase*k, width=1)
		self._damier.pack()
